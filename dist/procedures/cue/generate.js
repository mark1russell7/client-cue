...
import { loadFeatures, loadDependencies, resolveFeatures, checkCue, determineTsconfig, featureToFieldName, packageRoot, fileExists, readJson, writeJson, writeFile, mkdir, } from "../../shared.js";
/**
 * Generate package.json via CUE evaluation
 */
async function generatePackageJson(resolvedFeatures, existingPkg, ctx) {
 ...
 */
async function generateGitignore(resolvedFeatures, ctx) {
 ...
 */
async function setupCueMod(projectPath, ctx) {
    const cueModPath = resolve(projectPath, "cue.mod");
    if (!(await fileExists(cueModPath, ctx))) {
        await mkdir(cueModPath, ctx);
    }
    await writeFile(resolve(cueModPath, "module.cue"), `module: "project.local"
language: {
\tversion: "v0.15.1"
}
`, ctx);
 ...
 */
export async function cueGenerate(input, ctx) {
    ...
    const existingPkg = await readJson(resolve(projectPath, "package.json"), ctx);
